...
fn generate_file_queue_with_src_map(
    ...
    let mut file = match parse_file(src, cache) {
        Ok(file) => file,
    ...
    let mut sub_files = match file.block.get_import_files(src.val, cache, src_map) {
            ...
            Statement::Import(import) => match src::get_import_src(src, import) {
                Ok(import_src) => {
                    ...
                    if src_map.contains(&import_src) {
                        ...
                        generate_file_queue_with_src_map(import_src, cache, src_map)
                    }
                }
